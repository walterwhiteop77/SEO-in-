...
mdb = Database(DATABASE_URI, "admin_database")        results = await self.col.aggregate(pipeline).to_list(limit)
        return [result["_id"] for result in results]
...
mdb = Database(DATABASE_URI, "admin_database")        await self.col.delete_many({})
